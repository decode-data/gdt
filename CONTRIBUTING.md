...
- **Consumers pin by tag, not `main`:** e.g. fetch or vendor `schema/gdt-v0.1.schema.json` from `https://raw.githubusercontent.com/decode-data/gdto/vX.Y.0/schema/gdt-v0.1.schema.json`, or reference the tag/release directly. `madflow-sqlops`'s own README already states this intent ("Pin the GDT schema version explicitly... don't fetch it dynamically at runtime").
